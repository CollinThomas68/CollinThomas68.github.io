...
ajaxGet(`http://localhost:3000/api/teddies/${id}`, function (reponse) {
    ...
    if(id==teddy._id){
       ...
       */
        ...
        couleurElt.innerHTML='<label for="couleurs">Couleur</label><select id="couleurs" name="couleurs"></select>';
        console.log(couleurElt);
        var teddyCouleursElt = document.getElementById("qt");
        console.log(teddyCouleursElt);
        /*couleurSelectElt.classList.add("choixcouleurs");*/
        /*for (let couleur of teddyCouleurs) {
 
            var choixCouleurElt=document.createElement("option");
            choixCouleurElt.innerHTML=couleur;

        }
        teddyCouleurs.forEach(function(element){
            var choixCouleurElt=document.createElement("option");
            choixCouleurElt.innerHTML=element;
            teddyCouleursElt.appendChild(choixCouleurElt);
        });*/
